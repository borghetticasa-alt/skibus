...
export async function evaluateTrip(tripId: string, isAdminAction: boolean = false): Promise<DecisionResult> {
  ...
  for (const bus of trip.bus_runs) {
    ...
    const bep = calculateBreakEven(trip.fixed_cost, bus.cost, trip.base_price, trip.commission_rate);
    ...
    busEvaluations.push({
      ...
      isProfitable,
      breakEvenPoint: bep,
      projectedProfit: (seatsSold * trip.base_price * (1 - trip.commission_rate)) - (trip.fixed_cost + bus.cost)
    });
