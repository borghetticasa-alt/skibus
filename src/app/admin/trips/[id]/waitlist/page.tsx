...
type RiepilogoTripLayout = {
  nomeDestinazione: string;
  etichettaPartenza: string;
  stato: TripStatus;
  sla: { livello: SlaLevel; etichetta: string };
};
...
export default function WaitlistPage() {
  ...
  const [busOperativi, setBusOperativi] = useState<BusOperativo[]>([
    // Bus #1 pieno -> waitlist attiva
    { id: 'BUS-1', ordine: 1, tipo: 'GRANDE', nome: 'Bus grande', posti: 52, venduti: 52, stato: 'PIENO' },
  ]);

  // --- Coda (mock) ---
  const [coda] = useState<RigaCoda[]>([
  ...
  // Scelta bus minimo sufficiente: PICCOLO -> MEDIO -> GRANDE
  ...
  // --- Stato gita per TripLayout ---
  // Non indoviniamo i literal di TripStatus: usiamo un valore interno e lo castiamo UNA volta.
  const statoTrip: TripStatus = useMemo(() => {
    const statoInterno = 'IN_ATTESA'; // solo informativo qui
    return statoInterno as unknown as TripStatus;
  }, []);

  const riepilogo: RiepilogoTripLayout = useMemo(() => {
    const livello = ('VERDE' as unknown as SlaLevel);
    const etichetta = waitlistAttiva ? "Lista d'attesa attiva" : "Lista d'attesa non attiva";

    return {
      nomeDestinazione,
      etichettaPartenza: mounted
        ? `${new Date(dataPartenzaISO).toLocaleString('it-IT', {
            day: '2-digit',
            month: 'short',
            year: 'numeric',
            hour: '2-digit',
  ...
  }, [mounted, nomeDestinazione, dataPartenzaISO, statoTrip, waitlistAttiva]);
  ...
  return (
  <TripLayout id={id} activeTab="waitlist">
      <div className="space-y-8">
        ...
        <div className="bg-slate-950/30 rounded-[32px] border border-white/10 p-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              ...
              <h2 className="text-2xl font-black tracking-tight text-white mt-2">Lista d’attesa = bus aggiuntivi (2°, 3°, 4°…)</h2>
              <p className="text-sm font-semibold text-slate-300/70 mt-2 leading-relaxed max-w-3xl">
                Regola: <b>finché il bus #1 non è pieno</b>, la lista d’attesa è spenta. Quando il bus #1 è pieno, la domanda extra va in coda
                e serve per decidere se attivare un <b>bus aggiuntivo</b> (piccolo → medio → grande).
              </p>
            </div>
            ...
            return (
              <div key={b.id} className="bg-slate-950/30 rounded-[32px] border border-white/10 p-8">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Bus #{b.ordine}</div>
                    <div className="mt-2 text-xl font-black text-white tracking-tight">{b.nome}</div>
        ...
        {/* Blocco waitlist */}
        <div className="rounded-[32px] border p-8 bg-slate-950/30 border-white/10">
          <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
            <div>
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Lista d’attesa</div>
            ...
            <div className="bg-white/50 text-white rounded-[28px] p-6 min-w-[280px]">
              <div className="text-[10px] font-black uppercase tracking-widest text-white/60">Domanda in coda</div>
          ...
          {/* Proposta bus + tabella */}
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className={`rounded-[28px] border p-6 ${waitlistAttiva ? 'bg-slate-950/30 border-white/10' : 'bg-slate-950/30 border-white/10 opacity-50'}`}>
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Proposta bus aggiuntivo</div>

              <div className="mt-3 text-lg font-black text-white">{propostaBusAggiuntivo ? propostaBusAggiuntivo.nome : 'Nessuna proposta'}</div>
              ...
              <div className="mt-3 text-xs font-semibold text-slate-400">
                Regola scelta bus: <b className="text-white">piccolo → medio → grande</b>.
              </div>
            </div>

            <div className={`rounded-[28px] border p-6 ${waitlistAttiva ? 'bg-slate-950/30 border-white/10' : 'bg-slate-950/30 border-white/10 opacity-50'}`}>
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Dettaglio coda</div>
              ...
              <div className="mt-4 text-xs font-semibold text-slate-400 leading-relaxed">
                In questa fase i record restano “IN_CODA” come memoria domanda. Nella fase successiva li useremo per inviti/pagamenti mirati sul bus #2.
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 bg-white/50 text-white rounded-[28px] p-6">
            ...
            <div className="mt-3 text-sm font-semibold leading-relaxed text-white/85">
              Le prenotazioni si fanno <b>solo pagando</b>. Se il bus è pieno, puoi entrare in <b>lista d’attesa</b>: se si raggiunge abbastanza richiesta,
              attiviamo un bus aggiuntivo per la stessa data e modalità.
            </div>
          </div>
        </div>
      </div>
