...
import React, { useState, useMemo, useEffect, use as usePromise } from 'react';
...
export default function WaitlistPage({ params }: { params: Params | Promise<Params> }) {
  ...
  const [activeModal, setActiveModal] = useState<{
    type: 'INVITE' | 'EXPIRE' | 'BATCH_INVITE' | 'ACTIVATE_VEHICLE';
  ...
  const forceCleanExpired = () => {
    ...
    setData((prev) => ({
      ...prev,
      waitlist: prev.waitlist.map((e) =>
        e.status === 'INVITED' && e.inviteExpiresAt && e.inviteExpiresAt < nowISO ? { ...e, status: 'EXPIRED' } : e
      ),
    }));
  };

  return (
    <TripLayout id={tripId} activeTab="waitlist" tripSummary={data.summary}>
          ...
          <div className={`${decisionStats.suggestion.color} col-span-2 rounded-[40px] p-10 text-white shadow-xl relative overflow-hidden group transition-all duration-500`}>
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
              ...
              <div className="bg-white/10 backdrop-blur-md p-8 rounded-[32px] border border-white/10 min-w-[260px]">
                ...
                <button
                  onClick={() =>
                    decisionStats.suggestion.vehicle &&
                    setActiveModal({ type: 'ACTIVATE_VEHICLE', entries: [], suggestedVehicle: decisionStats.suggestion.vehicle })
                  }
                  ...
                  <tr key={entry.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-8 py-6">
                      ...
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                        Iscritto il {new Date(entry.createdAt).toLocaleDateString()}
                      </div>
                    </td>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-3">
                      ...
                      <p className="text-[11px] text-slate-400 leading-relaxed">
                        <b className="text-white uppercase tracking-tighter">IN_QUEUE:</b> Domanda grezza. L'utente attende lo sblocco logistico del mezzo.
                      </p>
                    </div>
                    <div className="flex gap-3">
                      <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-1.5 shrink-0" />
                      <p className="text-[11px] text-slate-400 leading-relaxed">
                        <b className="text-white uppercase tracking-tighter">INVITED:</b> Finestra di acquisto 24h aperta. Posto pre-riservato logicamente.
                      </p>
                    </div>
                    <div className="flex gap-3">
                      <div className="w-1.5 h-1.5 rounded-full bg-rose-500 mt-1.5 shrink-0" />
                      <p className="text-[11px] text-slate-400 leading-relaxed">
                        <b className="text-white uppercase tracking-tighter">EXPIRED:</b> Mancato acquisto o rimozione manuale. Pax liberato dal BEP.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-[48px] shadow-2xl max-w-lg w-full p-12 animate-in slide-in-from-bottom-8 duration-500 relative">
            ...
            <div
              className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-8 ${
                activeModal.type === 'EXPIRE' ? 'bg-rose-50 text-rose-600' : 'bg-indigo-50 text-indigo-600'
              }`}
            >
              {activeModal.type === 'ACTIVATE_VEHICLE' ? (
                <ArrowUpRight size={40} />
              ) : activeModal.type === 'EXPIRE' ? (
                <AlertCircle size={40} />
              ) : (
            ...
            <div className="mb-8">
              <h3 className="text-3xl font-black text-slate-900 mb-2 tracking-tighter">
                {activeModal.type === 'ACTIVATE_VEHICLE'
                  ? `Attiva ${activeModal.suggestedVehicle}`
                  : activeModal.type === 'INVITE'
                  ? 'Invia Invito'
                  : activeModal.type === 'BATCH_INVITE'
                  ? 'Invito Batch'
                  : 'Segna Scaduto'}
              </h3>
            ...
            <div className="space-y-8">
              <div>
                ...
                <textarea
                  ...
                  className={`w-full bg-slate-50 border rounded-3xl p-6 text-sm font-medium outline-none h-32 resize-none transition-all focus:ring-4 ${
                    error ? 'border-rose-300 ring-rose-50' : 'border-slate-200 focus:ring-indigo-50'
                  }`}
