...
export default function BusesPage({ params }: { params: { id: string } }) {
  const [buses, setBuses] = useState<BusRunDTO[]>(BUSES_MOCK);
  ...
  return (
    <TripLayout id={params.id} activeTab="buses" tripSummary={tripSummary}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.15em] mt-1">Configurazione Mezzi Monte Rosa Bus</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {hasMidibus && (
              <button 
                onClick={() => setActiveModal({ type: 'UPGRADE', bus: buses.find(b => b.type === 'MIDIBUS') })}
        ...
        <div className="grid gap-6">
          {buses.map((bus) => (
            <div key={bus.id} className="bg-white rounded-[40px] border border-slate-200 p-8 shadow-sm transition-all hover:border-indigo-100 hover:shadow-md relative group">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10">

                {/* Visual Identity */}
                <div className="flex items-center gap-6">
                  <div className={`w-20 h-20 rounded-[28px] flex items-center justify-center transition-colors ${bus.type === 'COACH' ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600'}`}>
                    <BusIcon size={36} />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-12 flex-1 max-w-xl">
                  <div className="flex-1">
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <span className="text-sm font-black text-slate-700">{bus.sold} / {bus.capacity} pax</span>
                    </div>
                    ...
                    <div className="flex justify-between mt-2">
                       <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">BEP: {bus.breakEvenSeats} pax</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[48px] shadow-2xl max-w-lg w-full p-12 animate-in slide-in-from-bottom-8 duration-500 relative overflow-hidden">
            ...
            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-8 ${
              activeModal.type === 'OVERRIDE' || activeModal.type === 'REMOVE' ? 'bg-rose-50 text-rose-600' : 'bg-indigo-50 text-indigo-600'
            }`}>
              {activeModal.type === 'OVERRIDE' ? <Settings2 size={40} /> : activeModal.type === 'REMOVE' ? <Trash2 size={40} /> : <ShieldAlert size={40} />}
            </div>

            <div className="mb-8">
              <h3 className="text-3xl font-black text-slate-900 mb-2 tracking-tighter">
                {activeModal.type === 'CONFIRM' ? 'Conferma Mezzo' : 
                 activeModal.type === 'LOCK' ? 'Lock Logistico' : 
                 activeModal.type === 'OVERRIDE' ? 'Override Capacità' : 
                 activeModal.type === 'REMOVE' ? 'Rimozione Mezzo' : 
                 activeModal.type === 'UPGRADE' ? 'Upgrade a Coach' : 'Nuovo Mezzo'}
              </h3>
            ...
            {activeModal.type === 'OVERRIDE' && (
              <div className="mb-8">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Capacità Target</label>
            ...
            <div className="space-y-8">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Nota Operativa Obbligatoria (min 10 char)</label>
                <textarea 
                  value={note}
              ...
              <div className="flex gap-4">
                ...
                <button 
                  ...
                  className={`flex-1 py-5 text-white rounded-2xl font-black text-sm shadow-2xl transition-all disabled:opacity-40 disabled:shadow-none ${
                    activeModal.type === 'OVERRIDE' || activeModal.type === 'REMOVE' ? 'bg-rose-600 shadow-rose-200' : 'bg-indigo-600 shadow-indigo-200'
                  }`}
                >
                  Esegui Comando
                </button>
