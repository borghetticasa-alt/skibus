 ...
 * - Quantità a 0 escluse dai calcoli economici
...
type TripNumbersDTO = {
  ...
  costs: {
    ...
    variablePerPersonEUR: number; // 0 = ignorato
  };
  ...
  categories: [
    {
      ...
      skipassFree: false,
      countsAsPerson: true,
      pricesByDate: { "2026-01-18": { group: 57, standard: 69 } },
    },
    {
      key: "SENIOR_65PLUS",
      ...
      skipassFree: false,
      countsAsPerson: true,
      pricesByDate: { "2026-01-18": { group: 53, standard: 65 } },
    },
    {
      key: "YOUNG_U24",
      ...
      skipassFree: false,
      countsAsPerson: true,
      pricesByDate: { "2026-01-18": { group: 49, standard: 60 } },
    },
    {
      key: "JUNIOR_U16",
      ...
      skipassFree: false,
      countsAsPerson: true,
      pricesByDate: { "2026-01-18": { group: 36, standard: 45 } },
    },
    {
      key: "BABY_U8_FREE",
      label: "Baby <8 (gratis)",
      ...
      skipassFree: true,
      countsAsPerson: true,
      pricesByDate: { "2026-01-18": { group: 0, standard: 0 } },
    },
    {
...
function toneForStatus(status: TripStatus) {
  ...
  if (status === "CANCELLED" || status === "FULL") return "danger" as const;
...
function parseISODate(s: string) {
  ...
  // mezzogiorno per evitare effetti timezone “strani”
...
export default function TripNumbersPage({
  ...
  // ✅ Next 16: params è Promise -> unwrap con use()
  const { id } = use(params);
  ...
  const totals = useMemo(() => {
    ...
    const threshold = data.rules.groupThreshold;

    const totalPeople = data.categories
      .filter((c) => c.countsAsPerson)
      .reduce((acc, c) => acc + clamp0(c.qty), 0);

    const priceMode: PriceMode = totalPeople >= threshold ? "GROUP" : "STANDARD";

    const revenueSkipass = data.categories.reduce((acc, c) => {
      const qty = clamp0(c.qty);
      if (qty === 0) return acc;       // qty 0 escluso
      if (c.skipassFree) return acc;   // baby gratis
    ...
    const costsTotal = fixedCosts + variable * totalPeople;
    ...
    const canEstimateMin = avgRevenuePerPerson > variable + 0.01;
    const minPeopleForTarget = canEstimateMin
      ? Math.ceil((fixedCosts + target) / (avgRevenuePerPerson - variable))
      : null;

    const missingToThreshold = Math.max(0, threshold - totalPeople);
  ...
  const toggleManualConfirmed = () => {
    setData((prev) => {
      const nextManual = !prev.controls.manualConfirmed;
      return {
        ...prev,
        controls: { ...prev.controls, manualConfirmed: nextManual },
        summary: {
          ...prev.summary,
          status: nextManual ? "CONFIRMED" : "SOFT_HOLD",
        },
      };
    });
  };

  const statusBadge: TripStatus = totals.confirmed ? "CONFIRMED" : data.summary.status;

  return (
    <TripLayout
      id={id}
      activeTab="numbers"
      tripSummary={{ ...data.summary, status: statusBadge }}
    >
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-3">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="mb-1 flex items-center gap-2 text-xs font-bold text-slate-500">
                  ...
                  <span
                    className="ml-1 inline-flex items-center text-slate-400"
                    title="Serve per calcolare il minimo stimato di partecipanti. Non mostrarlo ai clienti."
                  >
                    <Info size={14} />
                  </span>
                </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setData((prev) => ({
                        ...
                        controls: {
                          ...prev.controls,
                          targetMarginEUR: clamp0(Number(e.target.value)),
                        },
                      }))
                    }
                    className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-200"
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <div className="mt-1 text-xs font-semibold text-slate-500">Baby gratis inclusi (ricavo 0)</div>
            </CardContent>
          </Card>
          ...
          <CardContent className="p-5">
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="mt-1 text-xs text-slate-500">(solo admin, non mostrarlo ai clienti)</div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="mb-1 flex items-center gap-2 text-xs font-bold text-slate-500">
                  ...
                  <span
                    className="ml-1 inline-flex items-center text-slate-400"
                    title="Stima basata sul ricavo medio per persona del mix attuale. Se cambi quantità o prezzi, cambia anche la stima."
                  >
                    <Info size={14} />
                  </span>
                </div>
                ...
                {totals.minPeopleForTarget === null ? (
                  ...
                  <>
                    ...
                    <div className="mt-1 text-xs font-semibold text-slate-500">
                      ...
                      <span
                        className={
                          totals.missingToTarget && totals.missingToTarget > 0
                            ? "text-amber-700"
                            : "text-emerald-700"
                        }
                      >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setData((prev) => ({
                          ...
                          costs: {
                            ...prev.costs,
                            variablePerPersonEUR: clamp0(Number(e.target.value)),
                          },
                        }))
                      }
                      className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-200"
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50">
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="px-6 py-3 text-center text-[11px] font-bold text-slate-500">Q.tà</th>
                      <th className="px-6 py-3 text-right text-[11px] font-bold text-slate-500">Prezzo gruppo</th>
                      <th className="px-6 py-3 text-right text-[11px] font-bold text-slate-500">Prezzo standard</th>
                      <th className="px-6 py-3 text-right text-[11px] font-bold text-slate-500">Totale</th>
                    </tr>
                  </thead>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data.categories.map((c) => {
                      ...
                      const unit = totals.priceMode === "GROUP" ? clamp0(p.group) : clamp0(p.standard);
                        ...
                        <tr key={c.key} className="hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4 text-center">
                            <input
                              ...
                              className="w-20 rounded-xl border border-slate-200 px-2 py-2 text-center text-sm outline-none focus:ring-2 focus:ring-indigo-200"
                            />
                          </td>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="inline-flex items-center gap-2">
                              <input
                                ...
                                className="w-24 rounded-xl border border-slate-200 px-2 py-2 text-right text-sm outline-none focus:ring-2 focus:ring-indigo-200"
                          ...
                          <td className="px-6 py-4 text-right text-sm font-bold">
                            {rowTotal === 0 ? <span className="text-slate-500">—</span> : moneyEUR(rowTotal)}
                          </td>
                        </tr>
                      );
                    })}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <ul className="mt-2 space-y-1 text-sm text-slate-600">
                        ...
                        <li>• Quantità <span className="font-semibold">0</span> non entra nei calcoli (né ricavi né altro).</li>
                        <li>• Baby contano come persone (bus/logistica), ma skipass può essere <span className="font-semibold">€0</span>.</li>
                        <li>• “Minimo stimato” è una bussola: dipende dal mix attuale (quantità + prezzi).</li>
                      </ul>
                    </div>
                  </div>
                </div>
